...
<!-- TODO(지민님이 채워주세요): UseCase 도입 계획이 있으면 "UseCase 없이 Repository 직접 호출 → 🟡 지적" 으로 바꾸기 -->
...
- **과도한 Log.d/e** — 프로덕션 빌드에 민감 정보가 찍히는지 (토큰, userId 등)
  <!-- TODO(지민님이 채워주세요): 배포 빌드에서 Log 제거 강제 여부 (ProGuard 설정 있으면 Suggestion으로, 없으면 Should Fix로 올리기) -->
