...
/**
 * 디버그 빌드에서만 [StackTraceElement]를 기반으로 지정한 위치 정보가 포함된 지연 생성 WTF 메시지를 로그로 출력합니다.
 *
...
inline fun LinkuLog.wtf(
    ...
    Log.wtf(caller.getTag, getMsg(locate, msg))
}
...
/**
 * 디버그 빌드에서만 [StackTraceElement]를 기반으로 지정한 위치 정보와 [Throwable]이 포함된 지연 생성 WTF 메시지를 로그로 출력합니다.
 *
 * 태그는 파일 이름으로 설정되며, 지정한 위치 정보는 메시지 앞에 대괄호로 감싸 포함됩니다.
 *
 * @param caller 호출자 정보를 담고 있는 스택 트레이스 요소
...
inline fun LinkuLog.wtf(
    ...
    Log.wtf(caller.getTag, getMsg(locate, msg), tr)
}
